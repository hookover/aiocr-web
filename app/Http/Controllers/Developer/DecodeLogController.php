...
class DecodeLogController extends BaseController
{
    public function index(Request $request)
    {
        ...
        $wheres[] = ['developer_id', '=', Auth::user()->developer_id];    //当前用户
        // 状态
        if ($req_params['type']) {
        ...
        //性能优化
        $files = File::select(['id_a' ,'id_b' , 'server_id', 'path', 'status', 'result', 'report', 'updated_at', 'created_at', 'file_type_id', 'app_id', 'cost'])
            ->where($wheres)
            ->orderBy('id_a', 'desc')
